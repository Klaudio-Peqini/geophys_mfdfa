...
def load_input_series(path: str) -> pd.Series:
    ...
    s = pd.Series(df[value_col].astype(float).to_numpy(), index=idx, name=value_col)
    return s.sort_index()
...
def compute_windowed_metrics(x, time_index, qs, scales, poly_order, fit_range, window_size_bins, window_step_bins, var_floor):
    rows = []
    n = len(x)
...
def save_plot_fqs_with_fits(scales, qs, Fq, outpng, fit_range, title='MFDFA fluctuation functions'):
    ...
    for i, q in enumerate(qs):
        ...
        x_plot = np.log10(scales[ok])
        y_plot = np.log10(y[ok])
...
def save_plot_delta_alpha(dfw, outprefix):
    ...
    plt.figure(figsize=(12, 6))
    plt.plot(times, dfw['delta_alpha'], marker='s', lw=1.2)
    plt.xlabel('Window mid-time')
...
def parse_args():
    p = argparse.ArgumentParser(description='Optimized MFDFA + ACF analysis for earthquake catalog CSVs.')
    src = p.add_mutually_exclusive_group(required=True)
    src.add_argument('--catalog', type=str)
    src.add_argument('--input-series-csv', type=str)
    ...
    p.add_argument('--n_phase_surrogates', type=int, default=0)

    p.add_argument('--skip-plots', action='store_true', help='Avoid PNG generation to reduce I/O and wall time.')
    p.add_argument('--skip-series-export', action='store_true', help='Do not write binned_series.csv for each run when using cached input series.')
    return p.parse_args()
...
def main():
    ...
    if args.input_series_csv is not None:
        s = load_input_series(args.input_series_csv)
        timings['load_cached_series_sec'] = perf_counter() - t0
    else:
        t = perf_counter()
        cat = load_catalog(args.catalog)
        timings['load_catalog_sec'] = perf_counter() - t

        t = perf_counter()
        cat_f = filter_catalog(cat, mag_min=args.magmin, mag_max=args.magmax, start=args.start, end=args.end,
                               lat_min=args.latmin, lat_max=args.latmax, lon_min=args.lonmin, lon_max=args.lonmax)
        ...
        timings['build_series_sec'] = perf_counter() - t
    ...
    timings['preprocess_series_sec'] = perf_counter() - t
    ...
    if not args.skip_plots:
        save_plot_fqs_with_fits(scales_arr, qs, Fq, outdir / 'mfdfa_Fq.png', fit_range,
    ...
    if args.window_size_bins is not None:
        ...
        windowed_df = compute_windowed_metrics(x, s.index, qs, np.asarray(scales_arr, dtype=int), args.poly,
                                              fit_range, args.window_size_bins, step, args.var_floor)
        ...
        if not args.skip_plots:
            ...
            if {'window_mid_time', 'h_q0', 'h_q5'}.issubset(windowed_df.columns):
                pd.DataFrame({
                    'window_mid_time': pd.to_datetime(windowed_df['window_mid_time']),
                    'delta_h': windowed_df['h_q0'] - windowed_df['h_q5'],
                }).to_csv(outdir / 'delta_h_h0_h5.csv', index=False)
            if 'delta_alpha' in windowed_df.columns:
                pd.DataFrame({
                    'window_mid_time': pd.to_datetime(windowed_df['window_mid_time']),
                    'delta_alpha': windowed_df['delta_alpha'],
                }).to_csv(outdir / 'delta_alpha_windowed.csv', index=False)
        timings['windowed_block_sec'] = perf_counter() - t

    shuffle_summary = {}
    if args.n_shuffles > 0:
        t = perf_counter()
        shuffle_df = shuffled_multifractal_test(x, qs, np.asarray(scales_arr, dtype=int), args.poly, fit_range,
                                                args.n_shuffles, args.shuffle_seed, args.var_floor)
    ...
    if args.n_phase_surrogates > 0:
        t = perf_counter()
        phase_df = phase_randomized_multifractal_test(x, qs, np.asarray(scales_arr, dtype=int), args.poly, fit_range,
                                                      args.n_phase_surrogates, args.shuffle_seed + 1000, args.var_floor)
        ...
        if phase_widths.size:
            phase_summary = {
                ...
                'p_value_phase_width_ge_real': float(np.mean(phase_widths >= summ['spectrum_width_alpha'])),
            }
    ...
    meta = {
        ...
        'windowed_hq': {
            ...
            'window_count': int(len(windowed_df)) if not windowed_df.empty else 0,
